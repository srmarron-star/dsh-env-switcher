...
function ensureWindows(cfg) {
  const ps = [
    ...
    `$p=${cfg.winPort}; ` +
      ...
      `Start-Process -FilePath 'cmd.exe' -ArgumentList '/c npx -y @deepseek-ai/dsh@0.1.1-rc.1 web --port $p' ` +
...
function ensureWsl(cfg) {
  const bashCmd =
    ...
    `setsid nohup dsh web --port ${cfg.wslPort} --no-open >${cfg.wslLog} 2>&1 < /dev/null & fi; sleep 3`;
