 ...
 * Injects a "环境" tab (conversation.view, order 30) and, via DOM graft, a
 * "运行环境" entry in the sidebar main column (above the workspace block,
      ...
      for (const key of ["windows", "wsl"]) {
        ...
        b.addEventListener("click", () => {
          ...
          fetch("/env/api/switch?target=" + key)
            .then((r) => r.json())
            .then((d) => {
              const dest = d.url || (key === "windows" ? "http://127.0.0.1:3080" : "http://127.0.0.1:3081");
              setTimeout(() => { window.location.href = dest; }, 600);
            })
